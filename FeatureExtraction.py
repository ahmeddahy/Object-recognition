...
class PCA:
    ...
    def ReadImages(self,operation):
        ...
        for Class in range(0,self.NumberOfClasses,1):
         path="C:/Users/dell/PycharmProjects/ObjectRecognition/"+operation+"/"+str(Class+1)+"/*.jpg"
              ...
              #  total = network[n].output[0] *self.weight[0][0]
                for i in range(0,len(network[0].output),1):
                    for j in range(0,len(network[0].input),1):
                        network[n].output[i]=network[n].output[i]+(self.OldWeight[i][j]*network[n].input[j])  # calc output
                        #if(j!=0):
                           # total=total+network[n].weight[i-1][j]
                        self.weights[i][j]=((network[n].output[i]*network[n].input[j])-(network[n].output[i]*self.calculate(i,j,network[n].output)))*learningrate
